...
void UART_Init(UART_TypeDef *UARTx, UART_CFG_Type *UART_ConfigStruct)
{
	...
	if(UARTx == UART0)
	{
		 ...
		 * is set to FCCLK / 2 */
		CLKPWR_SetPCLKDiv(CLKPWR_PCLKSEL_UART0, CLKPWR_PCLKSEL_CCLK_DIV_2);
	}
#endif
	...
	if(((UART1_TypeDef *)UARTx) == UART1)
	{
		...
		/* As default, peripheral clock for UART0 module
		 * is set to FCCLK / 2 */
		CLKPWR_SetPCLKDiv(CLKPWR_PCLKSEL_UART1, CLKPWR_PCLKSEL_CCLK_DIV_2);
	}
#endif
	...
	if(UARTx == UART3)
	{
		...
		/* As default, peripheral clock for UART0 module
		 * is set to FCCLK / 2 */
		CLKPWR_SetPCLKDiv(CLKPWR_PCLKSEL_UART3, CLKPWR_PCLKSEL_CCLK_DIV_2);
	}
#endif
...
//	CHECK_PARAM(PARAM_UART1_RS485_CFG_DIRCTRLPOL(RS485ConfigStruct->DirCtrlPol_Level));
//	CHECK_PARAM(PARAM_UART1_RS485_CFG_DIRCTRLSEL(RS485ConfigStruct->DirCtrlSel_Pin));
...
//		if (RS485ConfigStruct->DirCtrlSel_Pin == UART1_RS485_CFG_DIRCTRLSEL_DTR)
//		{
...
//		}
	...
	if (RS485ConfigStruct->NormalMultiDropMode_State == DISABLE)
	{
		tmp |= UART1_RS485CTRL_NMM_DIS;
	}
