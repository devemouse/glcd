 ...
 * @{
 */
const PINSEL::CFG_Type ssp0_sck_pin[SSP0_MAX_SCK_PIN] = { \
...
Status SSP::SetClock ( uint32_t target_clock)
{
    ...
    if (SSPx == LPC_SSP0)
    {
        ssp_clk = CLKPWR::GetPCLK (CLKPWR_PCLKSEL_SSP0);
    }
    else if (SSPx == LPC_SSP1)
    {
        ssp_clk = CLKPWR::GetPCLK (CLKPWR_PCLKSEL_SSP1);
    }
...
{
   PINSEL::CFG_Type default_pincfg;
    ...
    if (SSPx == LPC_SSP0)
    {
        ...
        // SCK pin
        PINSEL SCKpin(const_cast<PINSEL::CFG_Type *>(&ssp0_sck_pin[SSPPinCfg->SCK_Pin]));

        // SSEL pin
        if (SSPPinCfg->SSPMode == SSP_SLAVE_MODE)
        {
           PINSEL SSELpin(const_cast<PINSEL::CFG_Type *>(&ssp0_ssel_pin[SSPPinCfg->SSEL_Pin]));
        }
        else
        {
            if (SSPPinCfg->CSPinConfig == SSP_CS_GPIO)
            {
                ...
                default_pincfg.Funcnum = PINSEL::FUNC_0;
                // pull up resistor
                default_pincfg.Pinmode = PINSEL::PINMODE_PULLUP;
                // Normal
                default_pincfg.OpenDrain = PINSEL::PINMODE_NORMAL;
                // Apply these configuration above to CS pin
                PINSEL SSELpin(&default_pincfg);
            }
            else
            {
               PINSEL SSELPin(const_cast<PINSEL::CFG_Type *>(&ssp0_ssel_pin[SSPPinCfg->SSEL_Pin]));
        ...
        // MISO pin
        PINSEL MISOPin(const_cast<PINSEL::CFG_Type *>(&ssp0_miso_pin[SSPPinCfg->MISO_Pin]));

        // MOSI pin
        PINSEL MOSIPin(const_cast<PINSEL::CFG_Type *>(&ssp0_mosi_pin[SSPPinCfg->MOSI_Pin]));
    }
#endif
    ...
    if (SSPx == LPC_SSP1)
    {
        ...
        // SCK pin
       PINSEL SCKPin(const_cast<PINSEL::CFG_Type *>(&ssp1_sck_pin[SSPPinCfg->SCK_Pin]));

        // SSEL pin
        if (SSPPinCfg->SSPMode == SSP_SLAVE_MODE)
        {
           PINSEL SSELPin(const_cast<PINSEL::CFG_Type *>(&ssp1_ssel_pin[SSPPinCfg->SSEL_Pin]));
        }
        else
        {
            if (SSPPinCfg->CSPinConfig == SSP_CS_GPIO)
        ...
        // MISO pin
        PINSEL MISOPin(const_cast<PINSEL::CFG_Type *>(&ssp1_miso_pin[SSPPinCfg->MISO_Pin]));

        // MOSI pin
        PINSEL MOSIPin(const_cast<PINSEL::CFG_Type *>(&ssp1_mosi_pin[SSPPinCfg->MOSI_Pin]));
    }
#endif
}
...
void SSP::DeInit()
{
    ...
    if (SSPx == LPC_SSP0)
    {
       ...
       CLKPWR::ConfigPPWR (CLKPWR_PCONP_PCSSP0, DISABLE);
    }
#endif
    ...
    if (SSPx == LPC_SSP1)
    {
       ...
       CLKPWR::ConfigPPWR (CLKPWR_PCONP_PCSSP1, DISABLE);
    }
#endif
}
...
void SSP::Init( SSP_CFG_Type *SSP_ConfigStruct)
{
    ...
    {
        /* Set up clock and power for SSP0 module */
       CLKPWR::ConfigPPWR (CLKPWR_PCONP_PCSSP0, ENABLE);
    ...
    {
       /* Set up clock and power for SSP1 module */
       CLKPWR::ConfigPPWR (CLKPWR_PCONP_PCSSP1, ENABLE);
       /* As default, peripheral clock for SSP1 module
        * is set to FCCLK / 2 */
       CLKPWR::SetPCLKDiv(CLKPWR_PCLKSEL_SSP1, CLKPWR_PCLKSEL_CCLK_DIV_2);

       defaultSSPPinCfg.MISO_Pin = SSP1_MISO_P0_8;
       defaultSSPPinCfg.MOSI_Pin = SSP1_MOSI_P0_9;
       defaultSSPPinCfg.SCK_Pin = SSP1_SCK_P0_7;
       defaultSSPPinCfg.SSEL_Pin = SSP1_SSEL_P0_6;
       defaultSSPPinCfg.SSPMode = SSP_MASTER_MODE;
       defaultSSPPinCfg.CSPinConfig = SSP_CS_DEFAULT;
    }
#endif
    ...
    SetClock(SSP_ConfigStruct->ClockRate);
}
